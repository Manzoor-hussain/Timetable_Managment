...
#     return False
# def check_courses_to_add(request,name,day,start_time,end_time,cours,check):
#     days_2 = {"SUN":"SUNDAY", "MON":"MONDAY", "TUE":'TUESDAY', "WED":"WEDNESDAY", "THU":'THURSDAY', "FRI":'FRIDAY'}
#     full_day=""
#     if day in days_2:
#         full_day=days_2[day]
...
#     allstudent=Allstudent.objects.filter(user=request.user) 
#     student = Allcourse.objects.filter(allstudent__user=request.user,day=day)
#     if not constraint_obj and not student:
#         already_cons=Allstudent.objects.filter(user=request.user, course=cours, title=check)
#         if already_cons:
#             return False
#         else:
#             already_cons=Allstudent.objects.create(user=request.user, course=cours, title=check)
#             return True
...
#                 already_cons=Allstudent.objects.create(user=request.user, course=cours, title=check)
#                 return True
#         elif end_time<=first.start_time:
#             already_cons=Allstudent.objects.filter(user=request.user, course=cours, title=check)
#             if already_cons:
#                 return False
#             else:
...
#                 if constraint1.end_time <= start_time <= constraint2.start_time and  constraint1.end_time <= end_time <= constraint2.start_time:
#                     already_cons=Allstudent.objects.filter(user=request.user, course=cours, title=check)
#                     if already_cons:
#                         return False
#                     else:
...
#         if not timetable:
#             check_courses_to_add(request,cours.name,cours.day,cours.start_time,cours.end_time,cours,check)
#         if dday == 1:
